...
if( ! class_exists( 'One_Page_Conference_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class One_Page_Conference_Control_Repeater extends WP_Customize_Control {
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'one-page-conference' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'one-page-conference' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {    
            ...
            wp_enqueue_script( 'one-page-conference-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'one-page-conference-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'one-page-conference' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'one-page-conference' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="one-page-conference-image-attachment" data-placeholder="<?php echo esc_attr( 'No Image Selected', 'one-page-conference' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php echo esc_attr( 'No Image Selected', 'one-page-conference' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'one-page-conference' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php echo esc_attr( 'Add Image', 'one-page-conference' ); ?>" data-alt-label="<?php echo esc_attr( 'Change Image', 'one-page-conference' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php echo esc_attr( 'Change Image', 'one-page-conference' ); ?>
    										<# } else { #>
    											<?php echo esc_attr( 'Add Image', 'one-page-conference' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="one-page-conference-file-attachment" data-placeholder="<?php echo esc_attr( 'No File Selected', 'one-page-conference' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php echo esc_attr( 'No File Selected', 'one-page-conference' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php echo esc_attr( 'Add File', 'one-page-conference' ); ?>" data-alt-label="<?php echo esc_attr( 'Change File', 'one-page-conference' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php echo esc_attr( 'Change File', 'one-page-conference' ); ?>
    										<# } else { #>
    											<?php echo esc_attr( 'Add File', 'one-page-conference' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'one-page-conference' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
