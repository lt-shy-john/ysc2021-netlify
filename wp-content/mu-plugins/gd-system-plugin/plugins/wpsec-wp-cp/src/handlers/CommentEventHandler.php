...
class CommentEventHandler extends EventHandler {
	...
	public function handle_comment_hook( $comment_id, $comment ) {
		...
		$this->send_event( self::$event_status_map[ $current_hook_name ], array( 'comment_author_ip' => $comment->comment_author_IP ), SiteUtil::get_site_origin(), $event_meta );
	}
	...
	public function handle_comment_approve_hook( $approved, $comment_data ) {
		...
		$response = $this->send_event( self::$event_approve_map[ $current_hook_name ], array( 'comment_author_ip' => $comment_data['comment_author_IP'] ), SiteUtil::get_site_origin(), $event_meta );
