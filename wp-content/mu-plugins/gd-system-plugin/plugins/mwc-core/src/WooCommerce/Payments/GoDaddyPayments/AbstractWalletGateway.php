...
abstract class AbstractWalletGateway extends WC_Payment_Gateway
{
    ...
    /** @var string "Book with $wallet Pay" button type */
    const BUTTON_TYPE_BOOK = 'BOOK';

    /** @var string "Buy with $wallet Pay" button type */
    const BUTTON_TYPE_BUY = 'BUY';

    /** @var string "Donate with $wallet Pay" button type */
    const BUTTON_TYPE_DONATE = 'DONATE';

    /** @var string "Check out with $wallet Pay" button type */
    const BUTTON_TYPE_CHECKOUT = 'CHECKOUT';

    /** @var string "Order with $wallet Pay" button type */
    const BUTTON_TYPE_ORDER = 'ORDER';

    /** @var string "Pay with $wallet Pay" button type */
    ...
    {
        $this->id = "godaddy-payments-{$this->walletId}";
    ...
    public function enqueueAdminScriptsAndStyles() : void
    {
        ...
        Enqueue::style()
            ...
            ->setSource(WordPressRepository::getAssetsUrl("css/{$this->walletId}-settings.css"))
            ->execute();
    }
    ...
    public function updateConfigurationFromSettings() : void
    {
        ...
        foreach ($settingKeys as $wcSettingKey => $configKey) {
            ...
            try {
                Configuration::set(sprintf('payments.%s.%s', StringHelper::camelCase($this->walletId), $configKey), $settingValue);
    ...
    {
        if ($order instanceof WC_Order && 'poynt' === $order->get_payment_method() && 'mwc_payments_'.StringHelper::snakeCase($this->walletId) === $order->get_created_via()) {
    ...
    protected function generate_parent_gateway_settings_html(string $key = '', array $data = []) : string
    {
        ...
        ob_start(); ?>
        <div id="woocommerce-godaddy-payments-<?php echo esc_attr($this->walletId); ?>-parent-gateway-settings-summary">
            <h3 class="wc-settings-sub-title" id="woocommerce_godaddy-payments-<?php echo esc_attr($this->walletId); ?>-general-settings"><?php echo esc_html_x('General', 'General settings', 'mwc-core'); ?></h3>
    ...
    {
        return empty(Configuration::get('payments.'.StringHelper::camelCase($this->walletId).'.enabledPages'));
    }

    /**
     * Determines whether the given product is supported by this wallet gateway.
     *
     * @param WC_Product|int $product product or ID
     * @return bool
     */
    public function isProductSupported($product) : bool
    {
        $supported = true;
        ...
        return apply_filters('mwc_payments_'.StringHelper::snakeCase($this->walletId).'_product_is_supported', $supported, $product);
    }

    /**
     * Determines whether the current cart contents are supported by this wallet gateway.
     *
     * @return bool
     */
    public function isCartSupported() : bool
    {
        $supported = true;
        ...
        return apply_filters('mwc_payments_'.StringHelper::snakeCase($this->walletId).'_cart_contents_are_supported', $supported);
    }
