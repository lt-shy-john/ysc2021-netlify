...
class GoDaddyPaymentsGateway extends AbstractPaymentGateway implements SubscriberContract
{
    ...
    public function __construct()
    {
        ...
        $this->method_description = sprintf(
            ...
            '<a href="https://www.godaddy.com/legal/agreements/commerce-services-agreement" target="_blank">', '</a>'
        );
     ...
     */
    protected function maybeRenderAccountDetails()
    {
        if (true !== Configuration::get('features.gdp_by_default.enabled')) {
            return;
        }
        $business = Poynt::getBusiness(); ?>
        <b><?php esc_html_e('Business Name:', 'mwc-core'); ?> </b><?php echo esc_html($business->getDoingBusinessAs()); ?><br /><br />
        <b><?php esc_html_e('Email:', 'mwc-core'); ?> </b>
        <?php
            echo sprintf(
                __('%1$s %2$sSwitch account%3$s', 'mwc-core'),
                esc_html($business->getEmailAddress()),
    ...
    public function processPayment(PaymentTransaction $transaction) : PaymentTransaction
    {
        ...
        if ($transaction->getSource() !== 'mwc_payments_apple_pay' && ! $transaction->shouldTokenize() && $paymentMethod && ! $paymentMethod->getId() && ! $paymentMethod->getCreatedAt()) {
