...
class SessionWalletRequestAdapter implements DataSourceAdapterContract
{
     ...
     *
     * @param WC_Cart $cart
     * @param ProductLineObject[] $products
     */
    public function __construct(WC_Cart $cart, array $products = [])
    {
     ...
     * {@inheritDoc}
     *
     * @param array<array<string, WC_Product|float>> $products
     *
     * @throws Exception
     */
    public function convertFromSource(array $products = []) : WalletRequestObject
    {
    ...
    protected function anyProductNeedsShipping() : bool
    {
        foreach ($this->products as $productLine) {
            if ($productLine->getProduct()->needs_shipping()) {
                return true;
            }
        }
    ...
    {
        return ! $this->hasProductsFromRequest() ? round((float) ArrayHelper::get($this->source->get_totals(), 'total', 0.00), 2) : 0;
    }
