...
final class SSO {
	...
	public function login_init() {
		...
		if ( ! $enabled || WP_DEBUG || ! Plugin::is_gd() || ! defined( 'GD_ACCOUNT_UID' ) || ! GD_ACCOUNT_UID || (bool) filter_input( INPUT_GET, 'wpaas-standard-login' ) || has_action( 'login_form' ) || $this->is_plugin_hooked_login_enqueue_scripts() ) {
	 ...
	 *
	 * @return boolean True when a plugin is hooked into 'login_enqueue_scripts', else false.
	 */
	private function is_plugin_hooked_login_enqueue_scripts() {

		global $wp_filter;

		if ( array_key_exists( 'login_enqueue_scripts', $wp_filter ) ) {
		...
		$data           = $wp_filter['login_enqueue_scripts']->callbacks[ key( $wp_filter['login_enqueue_scripts']->callbacks ) ];
		...
		if ( ! empty( $hooked_actions ) ) {
			...
			/**
			 * These plugin classes are expected to hook into 'login_enqueue_scripts' to customize the login experience.
			 *
			 * @var array
			 */
			$excluded_plugin_classes = (array) apply_filters(
			...
			$hooked_actions = array_diff( $hooked_actions, $excluded_plugin_classes );
